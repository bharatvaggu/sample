...
export default class JqxCheckBox extends React.Component {
    componentDidMount() {
        let  me = this,
        options = me.manageAttributes();
        me.createComponent(options);

        /** Pooja Iyer | 26th April 2018 | NEWIMS-255 - C3 | Template | Department | UI | Add/Edit Page
        * This patch is used to change the value of checkbox on click of label
        */
        /* if (JQXLite(me.componentSelector).jqxCheckBox('enableContainerClick') == false) {
            $(".jqxCheckboxLabel").on('click', (event) => {
                let targetId = "#"+event.target.htmlFor;
                if (JQXLite(targetId).jqxCheckBox('val') == true){
                    JQXLite(targetId).jqxCheckBox('uncheck');
                } else {
                    JQXLite(targetId).jqxCheckBox('check');
                }
            });
        } */
        /** Ends here */
            ...
            <div id={id} style={{ pointerEvents: 'none' }} >
                {
                    <span style = {{marginTop: '1px'}}>
                        <label className="jqxCheckboxLabel" style={{pointerEvents: 'auto' }} htmlFor={id}>{this.props.value}</label>
                    </span>
                }
            </div>
        )
