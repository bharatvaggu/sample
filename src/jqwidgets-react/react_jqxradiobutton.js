...
export default class JqxRadioButton extends React.Component {
    ...
    render() {
        ...
        return (
            /**
             * @updated by: Sushant Shinde
             * @since: 30-May-2018
             * Made changes for radiobutton selection only when clicked on radiobutton or label of radiobutton
             */
            <div id={id} style={{ pointerEvents: 'none' }}>
                <label className="jqxRadioButtonLabel" style={{ cursor: 'pointer', display: 'inline-flex', pointerEvents: 'auto' }} htmlFor={id}>{this.props.value}{this.props.children}</label>
            </div>
        )
    };
};
