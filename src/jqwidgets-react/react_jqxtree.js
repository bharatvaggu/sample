...
export default class JqxTree extends React.Component {
    ...
    manageAttributes() {
        // displayTotal key Added by Bharat on Date - 6th April 2018 for NEWCMMS-303
        // multiSelect key Added by Bharat on Date - 8th May 2018 for NEWCMMS-382
        // autoAdjustHeight key Added by Bharat on Date - 28th May 2018 for auto adjust height of nested drop down component
        let properties = ['animationShowDuration', 'animationHideDuration', 'allowDrag', 'allowDrop', 'checkboxes', 'dragStart', 'dragEnd', 'disabled', 'easing', 'enableHover', 'height', 'hasThreeStates', 'incrementalSearch', 'keyboardNavigation', 'rtl', 'selectedItem', 'source', 'toggleIndicatorSize', 'toggleMode', 'theme', 'width', 'displayTotal', 'multiSelect','autoAdjustHeight'];
        let options = {};
    ...
    // Added by Bharat on Date - 6th April 2018 for NEWCMMS-303
    displayTotal(arg) {
        if (arg !== undefined) {
            JQXLite(this.componentSelector).jqxTree('displayTotal', arg)
        } else {
            return JQXLite(this.componentSelector).jqxTree('displayTotal');
        }
    };
    // Added by Bharat on Date - 8th May 2018 for NEWCMMS-382
    multiSelect(arg) {
        if (arg !== undefined) {
            JQXLite(this.componentSelector).jqxTree('multiSelect', arg)
        } else {
            return JQXLite(this.componentSelector).jqxTree('multiSelect');
        }
    };
    // Added by Bharat on Date - 28th May 2018 for Auto adjust height of nested drop down component
    autoAdjustHeight(arg) {
        if (arg !== undefined) {
            JQXLite(this.componentSelector).jqxTree('autoAdjustHeight', arg)
        } else {
            return JQXLite(this.componentSelector).jqxTree('autoAdjustHeight');
        }
    };
    addBefore(item, id) {
    ...
    // Third and Fourth params added by Bharat on Date - 18th May 2018 for NEWCMMS-382
    checkItem(item, checked, owner, calledfrom) {
        JQXLite(this.componentSelector).jqxTree('checkItem', item, checked, owner, calledfrom);  
    };
    collapseAll() {
    ...
    // type and selecteditem params added by Bharat on Date - 8th May 2018 for NEWCMMS-382
    selectItem(item, type, selecteditem) {
        JQXLite(this.componentSelector).jqxTree('selectItem', item, type, selecteditem);  
    };
    uncheckAll() {
    ...
    // Added by Bharat on Date - 5th April 2018 for NEWCMMS-338
    calculateWidth() {
        JQXLite(this.componentSelector).jqxTree('_calculateWidth');
    };
    val(value) {
        if (value !== undefined) {
